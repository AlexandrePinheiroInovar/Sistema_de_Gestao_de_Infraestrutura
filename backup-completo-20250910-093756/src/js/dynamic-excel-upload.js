...
async function loadExistingDataOnPageLoad() {
    ...
    try {
        ...
        if (!enderecosSection) {
            console.log(
                '📍 [DYNAMIC-EXCEL-UPLOAD] Não estamos na seção de endereços, pulando carregamento'
            );
            return;
        }
        ...
        if (tbody && tbody.children.length > 0) {
            console.log(
                '✅ [DYNAMIC-EXCEL-UPLOAD] Sistema principal já carregou dados, não interferindo'
            );
            return;
        }
        ...
        console.log(
            '🔄 [DYNAMIC-EXCEL-UPLOAD] Sistema principal não carregou dados, carregando...'
        );
...
async function handleExcelFileSelection(event) {
    ...
    try {
        ...
        if (data.length > BATCH_SIZE) {
            ...
            showNotification(
                '⚠️ Aviso',
                `Processando apenas ${BATCH_SIZE} registros para economizar cota do Firebase.`,
                'warning'
            );
        }
        ...
        showNotification(
            '✅ Sucesso!',
            `Upload concluído! ${data.length} registros importados e tabela atualizada.`,
            'success'
        );
    } catch (error) {
...
async function adaptTableToExcelColumns(data) {
    ...
    if (data._originalHeaders) {
        ...
        console.log(
            '💾 [DYNAMIC-EXCEL-UPLOAD] Ordem das colunas armazenada globalmente:',
            window.lastExcelColumnOrder
        );
    }
...
function updateTableHeader(columns) {
    ...
    console.log(
        '🏗️ [DYNAMIC-EXCEL-UPLOAD] Construindo cabeçalho com',
        columns.length,
        'colunas:',
        columns
    );
...
function updateTableBody(data, columns) {
    ...
    console.log(
        '🏗️ [DYNAMIC-EXCEL-UPLOAD] Renderizando',
        data.length,
        'linhas com',
        columns.length,
        'colunas cada'
...
async function saveDataToFirestoreOptimized(data) {
    console.log(
        '💾 [DYNAMIC-EXCEL-UPLOAD] Salvamento otimizado - Processando',
        data.length,
        'registros...'
    );
    ...
    if (!user) {
        throw new Error(
            'Usuário precisa estar logado para usar o sistema. Por favor, faça login primeiro.'
        );
    }
    ...
    for (let i = 0; i < data.length; i += MICRO_BATCH_SIZE) {
        ...
        console.log(
            `📦 [DYNAMIC-EXCEL-UPLOAD] Processando lote ${Math.floor(i / MICRO_BATCH_SIZE) + 1} (${chunk.length} registros)...`
        );
        ...
        if (chunk.length > 0) {
            await batch.commit();
            console.log(
                `✅ [DYNAMIC-EXCEL-UPLOAD] Lote ${Math.floor(i / MICRO_BATCH_SIZE) + 1} salvo (${chunk.length} registros)`
            );
    ...
    console.log(
        `🎉 [DYNAMIC-EXCEL-UPLOAD] Upload otimizado concluído! ${totalSaved} registros salvos`
    );
    return totalSaved;
}
...
async function reloadTableFromFirestore(page = 1) {
    ...
    try {
        ...
        if (enderecos && enderecos.length > 0) {
            ...
            console.log(
                `✅ [DYNAMIC-EXCEL-UPLOAD] Página ${page} recarregada com ${enderecos.length} registros do Firestore`
            );
        } else {
            console.warn(
                '⚠️ [DYNAMIC-EXCEL-UPLOAD] Nenhum dado encontrado no Firestore para esta página'
            );
        ...
        if (
            page === 1 &&
            window.loadEnderecosTable &&
            typeof window.loadEnderecosTable === 'function'
        ) {
            // Não chama para evitar conflito, deixar comentado
            // await window.loadEnderecosTable();
            console.log(
                '✅ [DYNAMIC-EXCEL-UPLOAD] Sistema principal disponível (não chamado para evitar conflito)'
            );
        }
    } catch (error) {
        ...
        if (page === 1) {
            try {
                if (
                    window.FirestoreIntegration &&
                    typeof window.FirestoreIntegration.loadEnderecos === 'function'
                ) {
                    ...
                    updatePaginationControls();
                    console.log(
                        '✅ [DYNAMIC-EXCEL-UPLOAD] Fallback: recarregado com FirestoreIntegration'
                    );
                }
            } catch (fallbackError) {
...
async function loadDirectFromFirestore(page = 1, limit = recordsPerPage) {
    console.log(
        `📥 [DYNAMIC-EXCEL-UPLOAD] Carregando página ${page} do Firestore (${limit} registros)...`
    );
    ...
    try {
        ...
        console.log(
            `📋 [DYNAMIC-EXCEL-UPLOAD] Carregados ${enderecos.length} registros da página ${page}`
        );
        return enderecos;
    } catch (error) {
...
function updateTableWithFirestoreData(enderecos) {
    console.log(
        '📋 [DYNAMIC-EXCEL-UPLOAD] Atualizando tabela com dados do Firestore...',
        enderecos.length,
        'registros'
    );
    ...
    if (!enderecos || enderecos.length === 0) {
        tbody.innerHTML =
            '<tr><td colspan="100%" style="text-align: center; padding: 20px;">Nenhum registro encontrado nesta página</td></tr>';
        return;
    }
    ...
    const recordWithOrder = enderecos.find(
        record => record.columnOrder && Array.isArray(record.columnOrder)
    );

    if (recordWithOrder && recordWithOrder.columnOrder && recordWithOrder.columnOrder.length > 0) {
        console.log(
            '📊 [DYNAMIC-EXCEL-UPLOAD] Usando ordem das colunas do registro Firestore:',
            recordWithOrder.columnOrder
        );
        columns = recordWithOrder.columnOrder.filter(col => !excludeFields.includes(col));
    } else if (
        window.lastExcelColumnOrder &&
        Array.isArray(window.lastExcelColumnOrder) &&
        window.lastExcelColumnOrder.length > 0
    ) {
        console.log(
            '📊 [DYNAMIC-EXCEL-UPLOAD] Usando ordem das colunas do último upload:',
            window.lastExcelColumnOrder
        );
        ...
        console.log(
            '📊 [DYNAMIC-EXCEL-UPLOAD] Usando ordem das colunas dinâmicas:',
            dynamicTableColumns
        );
    ...
    console.log(
        '✅ [DYNAMIC-EXCEL-UPLOAD] Tabela atualizada com',
        enderecos.length,
...
window.deleteFirestoreRecord = async function (id) {
    ...
    try {
        ...
        if (window.deleteEndereco && typeof window.deleteEndereco === 'function') {
            ...
            window.FirestoreIntegration &&
            typeof window.FirestoreIntegration.deleteEndereco === 'function'
        ) {
...
function getCurrentUser() {
    ...
    if (
        ...
        if (user) {
            console.log(
                '👤 [DYNAMIC-EXCEL-UPLOAD] Usuário obtido via FirebaseAuthIsolated:',
                user.uid
            );
            return user;
        }
...
function updatePaginationControls() {
    console.log(
        `📄 [DYNAMIC-EXCEL-UPLOAD] Atualizando controles de paginação - Página ${currentPage}, Total: ${totalRecords}`
    );
...
window.confirmCleanupOldData = async function () {
    ...
    try {
        ...
        if (deleteCount > 0) {
            ...
            showNotification(
                '✅ Limpeza concluída!',
                `${deleteCount} registros antigos removidos.`,
                'success'
            );
        } else {
            showNotification(
                'ℹ️ Nada para limpar',
                'Não há registros antigos para remover.',
                'info'
            );
        }
...
console.log(
    '✅ [DYNAMIC-EXCEL-UPLOAD] Sistema dinâmico de upload Excel carregado com otimizações de cota'
);
