...
class UserDropdownSystem {
    ...
    initializeDisplay() {
        ...
        const initialAvatar = 'https://ui-avatars.com/api/?name=Carregando&background=64748b&color=fff&size=';
        
        if (userDisplayNameEl) userDisplayNameEl.textContent = initialName;
        if (userDisplayRoleEl) userDisplayRoleEl.textContent = initialRole;
        if (userAvatarEl) userAvatarEl.src = initialAvatar + '40';
        if (dropdownUserNameEl) dropdownUserNameEl.textContent = initialName;
        if (dropdownUserEmailEl) dropdownUserEmailEl.textContent = initialEmail;
        if (dropdownUserAvatarEl) dropdownUserAvatarEl.src = initialAvatar + '50';
        ...
        const setupAuthObserver = () => {
            if (typeof firebase !== 'undefined' && firebase.auth) {
                firebase.auth().onAuthStateChanged((user) => {
                    console.log('🔄 [USER-DROPDOWN] Mudança de autenticação detectada:', user ? user.email : 'logout');
                    if (user) {
    ...
    async loadUserData() {
        ...
        try {
            ...
            console.log('⏳ [USER-DROPDOWN] Dados do usuário não disponíveis no momento, observador cuidará das atualizações');
    ...
    async updateUserDisplay() {
        ...
        try {
            ...
            const avatarName = displayName.split(' ').map(n => n[0]).join('').substring(0, 2);
            ...
            if (userDisplayRoleEl) userDisplayRoleEl.textContent = this.formatRole(role);
            if (userAvatarEl) userAvatarEl.src = avatarUrl + '40';
            ...
            if (dropdownUserAvatarEl) dropdownUserAvatarEl.src = avatarUrl + '50';
