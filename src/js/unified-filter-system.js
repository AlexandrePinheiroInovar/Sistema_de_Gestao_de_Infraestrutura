...
class UnifiedFilterSystem {
    constructor() {
        ...
        this.filterConfig = [
            ...
            { id: 'infraFilterSubProjeto', name: 'subProjeto', label: 'Sub-Projeto', column: 'Sub Projeto' },
            { id: 'infraFilterTipoAcao', name: 'tipoAcao', label: 'Tipo de Ação', column: 'Tipo de Ação' },
            { id: 'infraFilterCondominio', name: 'condominio', label: 'Condomínio', column: 'Condominio' },
            { id: 'infraFilterSupervisor', name: 'supervisor', label: 'Supervisor', column: 'Supervisor' },
            ...
            { id: 'infraFilterPeriodoRecebimento', name: 'periodoRecebimento', label: 'Período de Recebimento', 
              dateColumns: ['DATA RECEBIMENTO', 'DATA INICIO', 'DATA FINAL'] } // Filtro especial para datas
        ];
        ...
        try {
            ...
            window.addEventListener('firebaseTableDataLoaded', (event) => {
                ...
                if (event.detail && event.detail.data) {
                    ...
                    console.log('✅ [UNIFIED-FILTER] Dados atualizados:', this.allData.length, 'registros');
    ...
    async loadFirebaseData() {
        ...
        try {
            ...
            if (window.tableData && Array.isArray(window.tableData) && window.tableData.length > 0) {
                ...
                console.log('✅ [UNIFIED-FILTER] Dados obtidos da variável global tableData:', this.allData.length, 'registros');
                this.createFilterInterface();
                return;
            }
            ...
            if (window.FirebaseTableSystem && window.FirebaseTableSystem.getData) {
                ...
                if (data && data.length > 0) {
                    this.allData = data;
                    console.log('✅ [UNIFIED-FILTER] Dados obtidos do FirebaseTableSystem:', this.allData.length, 'registros');
                    this.createFilterInterface();
                    return;
                }
            }
            ...
            if (window.firebase && window.firebase.firestore) {
                ...
                console.log('✅ [UNIFIED-FILTER] Dados carregados diretamente do Firebase:', this.allData.length, 'registros');
                this.createFilterInterface();
                
            } else {
                this.retryCount++;
                if (this.retryCount < this.maxRetries) {
                    console.warn(`⚠️ [UNIFIED-FILTER] Nenhum sistema de dados disponível, aguardando... (tentativa ${this.retryCount}/${this.maxRetries})`);
                    // Retry em 2 segundos
                    setTimeout(() => this.loadFirebaseData(), 2000);
                } else {
                    console.error('❌ [UNIFIED-FILTER] Máximo de tentativas excedido. Verifique se o sistema Firebase está carregando corretamente.');
                }
            }
    ...
    createDateFilter(config) {
        ...
        if (dataInicio && dataFim) {
            ...
            console.warn('⚠️ [UNIFIED-FILTER] Inputs de data não encontrados: infraFilterDataInicio, infraFilterDataFim');
        }
    }
    
    createMultiSelectDropdown(selectElement, config) {
        // Verificar se já foi criado para este elemento
        if (selectElement.nextSibling && selectElement.nextSibling.classList && 
    ...
    createClearButton() {
        ...
        const existingButton = document.querySelector('.unified-clear-filters-btn, .btn-filter-clear');
        if (existingButton) {
        ...
        if (!filterContainer) {
            filterContainer = document.querySelector('.filters-actions, .filter-section, .filtros-container, [class*="filter"]');
        }
        ...
        clearButton.innerHTML = '<span class="btn-icon">🔄</span><span class="btn-text">Limpar Filtros</span>';
        ...
        console.log('✅ [UNIFIED-FILTER] Botão "Limpar Filtros" inserido no container:', filterContainer.id || filterContainer.className);
    }
    ...
    applyFilters() {
        ...
        console.log('✅ [UNIFIED-FILTER] Filtros aplicados:', this.filteredData.length, 'de', this.allData.length, 'registros');
    ...
    updateInterface() {
        console.log('🔄 [UNIFIED-FILTER] Atualizando interface com', this.filteredData.length, 'registros filtrados');
        ...
        if (window.FirebaseTableSystem && window.FirebaseTableSystem.updateCards) {
            ...
            try {
                ...
                console.warn('⚠️ [UNIFIED-FILTER] Erro ao atualizar cards via FirebaseTableSystem:', error);
            }
        }
        ...
        if (window.ChartsDashboard && typeof window.ChartsDashboard.atualizar === 'function') {
            ...
            try {
                ...
                console.warn('⚠️ [UNIFIED-FILTER] Erro ao atualizar gráficos via ChartsDashboard:', error);
            }
        }
        ...
        if (typeof window.criarTodosGraficos === 'function') {
            ...
            try {
                ...
                console.warn('⚠️ [UNIFIED-FILTER] Erro ao atualizar gráficos via criarTodosGraficos:', error);
            }
        }
...
setTimeout(() => {
    ...
    if (unifiedFilterSystem && (!unifiedFilterSystem.allData || unifiedFilterSystem.allData.length === 0)) {
    ...
    if (infraSection && infraSection.classList.contains('active')) {
        ...
        if (window.FirebaseTableSystem && typeof window.FirebaseTableSystem.loadData === 'function') {
