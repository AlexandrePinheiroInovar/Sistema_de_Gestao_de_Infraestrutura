...
class FilterStateManager {
    ...
    loadState() {
        try {
            ...
            if (saved) {
                ...
                console.log('📥 [FILTER-STATE] Estado carregado do localStorage:', this.currentState);
            }
        } catch (error) {
    ...
    hasActiveFilters() {
        ...
        // Verificar arrays
        const arrayFilters = ['projeto', 'subProjeto', 'equipe', 'status', 'cidade', 'supervisor', 'tipoAcao', 'condominio'];
        const hasArrayFilters = arrayFilters.some(key => Array.isArray(state[key]) && state[key].length > 0);
        
        // Verificar datas
        const hasDateFilters = state.dataInicio !== '' || state.dataFim !== '';
        
        const result = hasArrayFilters || hasDateFilters;
        console.log('🔍 [FILTER-STATE] hasActiveFilters:', result, 'Arrays:', hasArrayFilters, 'Datas:', hasDateFilters, 'Estado:', state);
    ...
    getActiveFiltersCount() {
        ...
        // Contar datas
        if (state.dataInicio !== '') count++;
        if (state.dataFim !== '') count++;
...
function restoreFiltersToInterface(state) {
    ...
    multiSelectIds.forEach(id => {
        ...
        if (element && hasInstance) readyCount++;
    });
...
function restoreMultiSelectValues(selectId, values) {
    ...
    if (window.multiSelectInstances && window.multiSelectInstances[selectId]) {
        ...
        console.log(`🔧 [FILTER-STATE] Valores atuais antes da restauração:`, Array.from(instance.selectedValues));
        ...
        console.log(`✅ [FILTER-STATE] Multi-select ${selectId} restaurado com sucesso:`, Array.from(instance.selectedValues));
    } else {
        console.warn(`⚠️ [FILTER-STATE] Instância não encontrada para ${selectId}, usando fallback`);
        ...
        if (select) {
            Array.from(select.options).forEach(option => {
                ...
                if (shouldSelect) {
                    console.log(`🔧 [FILTER-STATE] Marcando opção como selecionada: ${option.value}`);
                }
            });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function tryRestoreFilters() {
        ...
        if (readyCount >= multiSelectIds.length * 0.8 || retryCount >= maxRetries) {
            ...
            console.log('⏳ [FILTER-STATE] Multi-selects não prontos, tentando novamente em 500ms...');
            setTimeout(tryRestoreFilters, 500);
        } else {
            console.warn('⚠️ [FILTER-STATE] Máximo de tentativas atingido, alguns multi-selects podem não ter sido inicializados');
        }
    }
