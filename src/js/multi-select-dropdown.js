...
class MultiSelectDropdown {
    ...
    saveOriginalOptions() {
        ...
        this.allOptions = Array.from(originalOptions)
            .map(option => ({
                value: option.value,
    ...
    syncWithFilterState() {
        ...
        if (!window.filterState) {
            console.log(
                '⚠️ [MULTI-SELECT] filterState não disponível ainda para:',
                this.element.id
            );
            return;
        }
        ...
        if (Array.isArray(savedValues) && savedValues.length > 0) {
            ...
            console.log(
                '✅ [MULTI-SELECT] Sincronização concluída:',
    ...
    syncValuesWithFilterState() {
        ...
        if (needsSync) {
            console.log(
                '🔄 [MULTI-SELECT] Sincronização rápida necessária para:',
                this.element.id,
                {
                    savedValues,
                    currentValues
    ...
    renderOptions() {
        ...
        if (selectedCount > 0) {
            console.log(
                `🎨 [MULTI-SELECT] renderOptions() chamado para ${this.element.id} com ${selectedCount} valores selecionados:`,
                Array.from(this.selectedValues)
            );
        }
    ...
    clearAll() {
        ...
        console.log(
            '🧹 [MULTI-SELECT] Valores atuais antes de limpar:',
            Array.from(this.selectedValues)
        );
        ...
        if (window.filterState) {
            ...
            if (filterName) {
                ...
                if (Array.isArray(savedValues) && savedValues.length > 0) {
                    console.log(
                        '🚫 [MULTI-SELECT] BLOQUEADO: Tentando limpar filtro com valores ativos no filterState!',
                        {
                            selectId: this.element.id,
                            filterName: filterName,
                            savedValues: savedValues,
                    ...
                    console.log(
                        '🔄 [MULTI-SELECT] Valores restaurados do filterState:',
                        Array.from(this.selectedValues)
                    );
                    return;
                }
