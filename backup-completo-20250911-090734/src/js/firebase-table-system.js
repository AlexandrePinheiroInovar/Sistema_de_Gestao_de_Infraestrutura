...
function extractTableColumns(sampleData) {
    ...
    // Colunas exatas conforme especificadas (31 colunas)
    firebaseTableColumns = [
        ...
        'dataFinal', 'dias', 'equipe', 'supervisor', 'status', 'rdo',
        'book', 'colaboradorBook', 'dataBook', 'projeto2', 'colaboradorProjeto', 
        'dataProjeto', 'justificativa', 'observacao', 'solucao', 'menorOuMaiorQue30'
    ];
...
// FUNÇÃO SIMPLES E DIRETA: MAPEAR NOMES DE COLUNAS HTML PARA NOMES DO FIRESTORE
function mapFieldValue(row, column) {
    // MAPEAMENTO COMPLETO: coluna HTML -> campo exato do Firestore
    const fieldMappings = {
        'projeto': 'Projeto',
        ...
        'rdo': 'RDO',
        'book': 'BOOK',
        'colaboradorBook': 'COLABORADOR/BOOK',
        'dataBook': 'DATA BOOK',
        'projeto2': 'PROJETO',
        'colaboradorProjeto': 'COLABORADOR/PROJETO',
        'dataProjeto': 'DATA/PROJETO',
        'justificativa': 'JUSTIFICATIVA',
        'observacao': 'Observação',
        'solucao': 'SOLUÇÃO',
        'menorOuMaiorQue30': '< OU > QUE 30'
    };

    // Buscar pelo campo mapeado
    const firestoreField = fieldMappings[column];
    if (firestoreField && row[firestoreField] !== undefined && row[firestoreField] !== null && row[firestoreField] !== '') {
        return row[firestoreField];
    }

    // Fallback: buscar diretamente pelo nome da coluna  
...
function criarModalHistorico(logDocs, recordId) {
    // Remover TODOS os modais existentes
    const modaisExistentes = document.querySelectorAll('#historicoModal, .modal[id*="historico"]');
